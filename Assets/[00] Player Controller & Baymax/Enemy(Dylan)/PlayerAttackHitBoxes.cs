...
public class PlayerAttackHitboxes : MonoBehaviour
{
    ...
    // Attack 2
    public void A2_HitOn()  { if (attack2HB) attack2HB.Begin(attackerOrigin ?? transform); }
    public void A2_HitOff() { if (attack2HB) attack2HB.End(); }

    // Attack 3 (two phases)
    public void A3_Sweep_On()  { if (attack3HB_Sweep) attack3HB_Sweep.Begin(attackerOrigin ?? transform); }
    public void A3_Sweep_Off() { if (attack3HB_Sweep) attack3HB_Sweep.End(); }

    public void A3_Slam_On()   { if (attack3HB_Slam) attack3HB_Slam.Begin(attackerOrigin ?? transform); }
    public void A3_Slam_Off()  { if (attack3HB_Slam) attack3HB_Slam.End(); }

    // Plunge
    public void Plunge_On()    { if (plungeHB) plungeHB.Begin(attackerOrigin ?? transform); }
    public void Plunge_Off()   { if (plungeHB) plungeHB.End(); }
}
